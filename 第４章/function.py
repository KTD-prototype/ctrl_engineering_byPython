...
def bodeplot_set(fig_ax, *args):
    ...
    # show legend
    if len(args) > 0:
        fig_ax[1].legend(loc = args[0]) # if argments are equal or more then 1, write in gain diagrams
    if len(args) > 1:
        fig_ax[0].legend(loc = args[1]) # if eq or more than 2, also write in phase diagram, too
